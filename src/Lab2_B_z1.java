...
class TancSoStudentite {
    //TODO: Definicija na globalni promenlivi i semafori

    //da vlezat paralelno ne povekje od 10 od ist pol i eden
    static Semaphore maski;
    static Semaphore zhenski;
    static Semaphore chupeHere;  //za signalizacija
    static Semaphore tanc;  //maksimum tri para vlevaat
    //static Semaphore lock; //ne e atomicna i ima spodelen resurs
    static Semaphore maskoReady;
    ...
    public void init() {
        ...
        zhenski = new Semaphore(10);
        chupeHere = new Semaphore(0);
        tanc = new Semaphore(3);
        //lock = new Semaphore(1);
        maskoReady = new Semaphore(0);
    }
    ...
    class Masko extends Thread {
        ...
        public void ucestvo() throws InterruptedException {
            ...
            maski.acquire();
            show.presobleci(); //atomicna metoda
            chupeHere.acquire();
            tanc.acquire();
            maskoReady.release();
            // samo maskoto go povikuva metodot tancuvaj
            show.tancuvaj();  //atomicna metoda
            tanc.release();
            maski.release();
        }
